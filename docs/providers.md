...
1. Add a `Provider` entry in `src/main.rs` with a stable lowercase ID, display name, environment variable, endpoint, and protocol metadata.
...
5. Run `cargo test`, `python3 -m pytest tests/ -q`, and review the documentation diff.
